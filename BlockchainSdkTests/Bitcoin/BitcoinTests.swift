...
class BitcoinTests: XCTestCase {
    ...
    func testBtcTxBuilder() {
        ...
        let buildToSignResult = txBuilder.buildForSign(transaction: transaction, sequence: 4294967290, sortType: .none)!
        sizeTester.testTxSizes(buildToSignResult)
        let signedTx = txBuilder.buildForSend(transaction: transaction, signatures: [signature1, signature2], sequence: 4294967290, sortType: .none)
