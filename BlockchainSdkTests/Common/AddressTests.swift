...
class AddressesTests: XCTestCase {
    ...
    func testTON(curve: EllipticCurve) {
        ...
        let oldAddressService = WalletCoreAddressService(coin: .ton)
        let newAddressService = TonAddressService()
        
        let walletPubkey1 = Data(hex: "e7287a82bdcd3a5c2d0ee2150ccbc80d6a00991411fb44cd4d13cef46618aadb")
        
        let expectedOldAddress1 = "EQBqoh0pqy6zIksGZFMLdqV5Q2R7rzlTO0Durz6OnUgKrYeu"
        XCTAssertEqual(try oldAddressService.makeAddress(from: walletPubkey1).value, expectedOldAddress1)
        
        let expectedNewAddress1 = "UQBqoh0pqy6zIksGZFMLdqV5Q2R7rzlTO0Durz6OnUgKrdpr"
        XCTAssertEqual(try newAddressService.makeAddress(from: walletPubkey1).value, expectedNewAddress1)
        
        let walletPubkey2 = Data(hex: "258A89B60CCE7EB3339BF4DB8A8DA8153AA2B6489D22CC594E50FDF626DA7AF5")
        
        let expectedOldAddress2 = "EQAoDMgtvyuYaUj-iHjrb_yZiXaAQWSm4pG2K7rWTBj9eOC2"
        XCTAssertEqual(try oldAddressService.makeAddress(from: walletPubkey2).value, expectedOldAddress2)
        
        let expectedNewAddress2 = "UQAoDMgtvyuYaUj-iHjrb_yZiXaAQWSm4pG2K7rWTBj9eL1z"
        XCTAssertEqual(try newAddressService.makeAddress(from: walletPubkey2).value, expectedNewAddress2)
        
        let walletPubkey3 = Data(hex: "f42c77f931bea20ec5d0150731276bbb2e2860947661245b2319ef8133ee8d41")
        
        let expectedOldAddress3 = "EQBm--PFwDv1yCeS-QTJ-L8oiUpqo9IT1BwgVptlSq3ts90Q"
        XCTAssertEqual(try oldAddressService.makeAddress(from: walletPubkey3).value, expectedOldAddress3)
        
        let expectedNewAddress3 = "UQBm--PFwDv1yCeS-QTJ-L8oiUpqo9IT1BwgVptlSq3ts4DV"
        XCTAssertEqual(try newAddressService.makeAddress(from: walletPubkey3).value, expectedNewAddress3)
        ...
        XCTAssertNil(try? oldAddressService.makeAddress(from: walletPubkey4))
        XCTAssertNil(try? newAddressService.makeAddress(from: walletPubkey4))
        ...
        XCTAssertNil(try? oldAddressService.makeAddress(from: walletPubkey5))
        XCTAssertNil(try? newAddressService.makeAddress(from: walletPubkey5))
        
        XCTAssertNil(try? oldAddressService.makeAddress(from: secpCompressedKey))
        XCTAssertNil(try? newAddressService.makeAddress(from: secpCompressedKey))
        
        XCTAssertNil(try? oldAddressService.makeAddress(from: secpDecompressedKey))
        XCTAssertNil(try? newAddressService.makeAddress(from: secpDecompressedKey))
        
        try XCTAssertEqual(addressesUtility.makeTrustWalletAddress(publicKey: walletPubkey1, for: blockchain), expectedNewAddress1)
    }
