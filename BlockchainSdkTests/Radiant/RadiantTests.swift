...
final class RadiantTests: XCTestCase {
    ...
    func testSignRawTransaction() throws {
        ...
        let txBuilder = try RadiantCashTransactionBuilder(
        ...
        let signatures = try hashesForSign.enumerated().compactMap { (index, digest) in
            /*
                let signature = privateKey.sign(digest: digest, curve: .secp256k1)!
             */
            
            let signature = Data(hexString: expectedSignatures[index])
            
            guard publicKey.verify(signature: signature, message: digest) else {
                throw "Failed verifyAsDER for Radiant blockchain testSignRawTransaction method"
            }
